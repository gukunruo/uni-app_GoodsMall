...
// 是否适配底部安全区域 defineProps接收父组件传来的数据
defineProps<{
  safeAreaInsetBottom?: boolean
}>()

// 获取屏幕边界到安全区域距离[页面形式购物车cartPage]
...
const selectedCartList = computed(() => {
  // 过滤得到所有选中的商品项
  return cartList.value.filter((value) => value.selected)
})
  ...
  <scroll-view enable-back-to-top scroll-y class="scroll-view" @scrolltolower="onScrolltolower">
    ...
    <template v-if="memberStore.profile">
        ...
        <uni-swipe-action>
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <view class="goods">
              ...
              <!-- 商品数量 -->
              <view class="count">
                <!-- 使用步进器插件 index步进器标识符 onChange输入框内容发生变化时触发  -->
            ...
            <!-- uni-swipe-action组件中的右侧具名插槽 实现删除按钮 -->
            <template #right>
      ...
      <view class="cart-blank" v-else>
        ...
        <!-- tabbar跳转需要指定open-type为switchTab -->
      ...
      <view
        ...
        class="toolbar"
        :style="{
          paddingBottom: safeAreaInsetBottom ? safeAreaInsets?.bottom + 'px' : 0,
        }"
      >
