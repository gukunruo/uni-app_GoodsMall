...
import type { OrderPreResult } from '@/types/order.d.ts'
import {
...
// 页面参数 除了可以接收父组件传来的参数 在页面跳转时还可以接收路径后面的query参数
const query = defineProps<{
  // skuId、count是立即购买订单所必须的参数
  skuId?: string
  count?: string
  // orderId订单id是再次购买订单所需的参数
  orderId?: string
}>()

// 获取所有预付订单、立即购买订单、再次购买订单
const orderPre = ref<OrderPreResult>()
const getMemberOrderPreData = async () => {
  // 如果有立即购买订单的订单数量和skuId
...
// 选择的收货地址 拿到的是一个包含地址信息的对象
const selectedAddress = computed(() => {
  // 通过仓库中管理的地址管理界面修改的地址【优先】 or 从预定订单的用户地址中抽出默认地址
...
<template>
  <scroll-view enable-back-to-top scroll-y class="viewport">
    <!-- 收货地址 有地址显示第一个navigator 没有则显示第二个 -->
    <navigator
